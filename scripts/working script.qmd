...
```{r}
flow_volume <- runif(min=0.05, max=0.85, n=100)
stage <- runif(min=1, max=15, n=100)
df <- data.frame(flow_volume = flow_volume, stage = stage)
```

## Import Function (includes error checking & looping)
...
```{r}
  #adding for loop (old code:velocity_ft_s <- flow_volume / stage )
  for (n in 1:100) {
  velocity_ft_s = df$flow_volume[n] / df$stage[n]
}
```
...
```{r}
bar_plot <- ggplot(df, aes(x = prediction, y = #####)) + 
  geom_bar()
```
